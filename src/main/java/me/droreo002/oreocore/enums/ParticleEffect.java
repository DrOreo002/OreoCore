...
public enum ParticleEffect {
     ...
     * <p>
     * This class is part of the <b>ParticleEffects Library</b> and follows the same usage conditions
     *
     * @author DarkBlade12
     * @since 1.7
     */
    ...
    public static final class OrdinaryColor extends ParticleColor {
         ...
         */
        OrdinaryColor(int red, int green, int blue) throws IllegalArgumentException {
            if (red < 0) {
    ...
    public static final class ParticlePacket {
         ...
         * @see #initialize()
         */
        ParticlePacket(ParticleEffect effect, float offsetX, float offsetY, float offsetZ, float speed, int amount, boolean longDistance, ParticleData data) throws IllegalArgumentException {
            initialize();
            if (speed < 0) {
        ...
        static void initialize() throws VersionIncompatibleException {
            ...
            try {
                version = Integer.parseInt(BukkitReflectionUtils.PackageType.getServerVersion().split("_")[1]);
                if (version > 7 && version < 13)
                    enumParticle = BukkitReflectionUtils.PackageType.MINECRAFT_SERVER.getClass("EnumParticle");
                else if (version > 13)
                    enumParticle = BukkitReflectionUtils.PackageType.MINECRAFT_SERVER.getClass("ParticleParam");
                Class<?> packetClass = BukkitReflectionUtils.PackageType.MINECRAFT_SERVER.getClass(version < 7 ? "Packet63WorldParticles" : "PacketPlayOutWorldParticles");
