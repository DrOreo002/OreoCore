...
public enum OSound {
    ...
    ENTITY_PLAYER_LEVELUP("same", "same", "same", "same", "same", "LEVEL_UP"),
    ...
    @Getter
    private static Map<OSound, Sound> sounds = new HashMap<>();
    @Getter
    private List<String> soundsList = new ArrayList<>();

    static {
        serverVersion = ServerUtils.getServerVersion();
    }

    OSound(String... sounds) {
        for (String str : sounds) {
            if (str.equals("same")) {
                soundsList.add(this.toString());
                continue;
            }
            soundsList.add(str);
        }
    ...
    /**
     * Match sound from string
     *
     * @param val The matching string
     * @return The sound, can be null
     */
    @Nullable
    public static OSound match(String val) {
        val = val.toUpperCase();
        for (OSound gs : OSound.values()) {
            List<String> snds = gs.getSoundsList();
            for (String str : snds) {
    ...
    public Sound parseSound() {
        ...
        switch (serverVersion.getBaseVersion()) {
            case "UNKNOWN":
            case "V1_8": {
                String val = this.getSoundsList().get(5);
                Sound soun;
                try {
                    soun = Sound.valueOf(val);
                } catch (IllegalArgumentException e) {
                    soun = this.tryGenerate();
                }
                if (soun == null) {
                    return null;
                }
            ...
            case "V1_9": {
                String val = this.getSoundsList().get(4);
                Sound soun;
                try {
                    soun = Sound.valueOf(val);
                } catch (IllegalArgumentException e) {
            ...
            case "V1_10": {
                String val = this.getSoundsList().get(3);
                Sound soun;
                try {
                    soun = Sound.valueOf(val);
                } catch (IllegalArgumentException e) {
            ...
            case "V1_12": {
                String val = this.getSoundsList().get(1);
                Sound soun;
                try {
                    soun = Sound.valueOf(val);
                } catch (IllegalArgumentException e) {
    ...
    /**
     * Try generate a sound
     *
     * @return The Sound
     */
    private Sound tryGenerate() {
        ...
        for (String str : this.getSoundsList()) {
            Sound s;
            try {
                s = Sound.valueOf(str);
            } catch (IllegalArgumentException e) {
